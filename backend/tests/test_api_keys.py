...
from app.core.deps import get_embed_workspace, resolve_embed_workspace
...
async def test_list_api_keys_hides_secret(client):
    ...
    workspace_id = await create_workspace(client, headers)
    await client.post(
        f"/api/v1/workspaces/{workspace_id}/api-keys", json={"name": "A"}, headers=headers
    )
    await client.post(
        f"/api/v1/workspaces/{workspace_id}/api-keys", json={"name": "B"}, headers=headers
    )

    response = await client.get(
        f"/api/v1/workspaces/{workspace_id}/api-keys", headers=headers
    )
...
async def test_api_keys_require_manager_role(client, session_factory):
    ...
    listed = await client.get(
        f"/api/v1/workspaces/{workspace_id}/api-keys", headers=member_headers
    )
...
async def test_api_keys_reject_non_member_and_anonymous(client):
    ...
    other = await client.get(
        f"/api/v1/workspaces/{workspace_id}/api-keys", headers=other_headers
    )
    client.cookies.clear()
    anonymous = await client.get(f"/api/v1/workspaces/{workspace_id}/api-keys")
...
async def test_update_api_key_toggles_active(client):
    ...
    workspace_id = await create_workspace(client, headers)
    created = await client.post(
        f"/api/v1/workspaces/{workspace_id}/api-keys", json={"name": "T"}, headers=headers
    )
...
async def test_update_api_key_scoped_to_workspace(client):
    first = await register_and_login(client, "keys-first@example.com")
    first_workspace = await create_workspace(client, first, name="First Co")
    key_id = (
        await client.post(
            f"/api/v1/workspaces/{first_workspace}/api-keys",
            json={"name": "First"},
            headers=first,
        )
    ).json()["id"]
    ...
    second_workspace = await create_workspace(client, second, name="Second Co")

    response = await client.patch(
        f"/api/v1/workspaces/{second_workspace}/api-keys/{key_id}",
        json={"is_active": False},
        headers=second,
    )
...
async def test_resolve_embed_workspace_valid_updates_last_used(client, session_factory):
    headers = await register_and_login(client, "embed-owner@example.com")
    owner_id = await user_id_for(client, headers)
    workspace_id = await create_workspace(client, headers)
    key, plaintext = await add_key(session_factory, workspace_id, owner_id)

    async with session_factory() as session:
        workspace = await resolve_embed_workspace(session, plaintext)
...
async def test_resolve_embed_workspace_does_not_stamp_recently_used(
...
async def test_get_embed_workspace_reads_header(client, session_factory):
    headers = await register_and_login(client, "embed-header@example.com")
    owner_id = await user_id_for(client, headers)
    workspace_id = await create_workspace(client, headers)
    _, plaintext = await add_key(session_factory, workspace_id, owner_id)

    async with session_factory() as session:
        workspace = await get_embed_workspace(plaintext, session)

    assert workspace.id == workspace_id


async def test_resolve_embed_workspace_missing(session_factory):
    async with session_factory() as session:
        with pytest.raises(HTTPException) as exc:
            await resolve_embed_workspace(session, None)

    assert exc.value.status_code == 401
    assert exc.value.detail["code"] == "EMBED_KEY_REQUIRED"


async def test_resolve_embed_workspace_invalid(session_factory):
    async with session_factory() as session:
        with pytest.raises(HTTPException) as exc:
            await resolve_embed_workspace(session, "jrk_not-a-real-key")

    assert exc.value.status_code == 401
    assert exc.value.detail["code"] == "INVALID_EMBED_KEY"


async def test_resolve_embed_workspace_disabled(client, session_factory):
    headers = await register_and_login(client, "embed-disabled@example.com")
    owner_id = await user_id_for(client, headers)
    workspace_id = await create_workspace(client, headers)
    _, plaintext = await add_key(session_factory, workspace_id, owner_id, active=False)

    async with session_factory() as session:
        with pytest.raises(HTTPException) as exc:
            await resolve_embed_workspace(session, plaintext)
